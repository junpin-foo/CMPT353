...
def pivot_months_pandas(data):
    ...
    grouped = pd.DataFrame.groupby(data, by = ["month", "name"])
    temp = grouped.aggregate('sum').reset_index()
    monthly = temp.pivot(index = 'name', columns = 'month', values = 'precipitation')

    grouped2 = pd.DataFrame.groupby(data, by = ["name", "month"])
    temp2 = grouped2.aggregate('count').reset_index() 
    temp2['count'] = temp2['station']
    counts = temp2.pivot(index = 'name', columns = 'month', values = 'count')
