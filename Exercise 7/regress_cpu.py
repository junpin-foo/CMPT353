...
def get_data(filename):
    ...
    sysinfo[y_column] = sysinfo[y_column].shift(periods=-1)
...
def get_trained_coefficients(X_train, y_train):
    ...
    X_poly = poly.fit_transform(X_train['temperature'].values.reshape(-1,1))
...
def smooth_test(coef, sysinfo, outfile):
    ...
    temp = np.delete(transition, 1, 0)
    transition = np.vstack([coef,temp])
